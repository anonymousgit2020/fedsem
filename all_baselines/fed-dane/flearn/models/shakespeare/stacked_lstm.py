...
from flearn.utils.tf_utils import process_sparse_grad, process_sparse_grad2
...
class Model(object):
    ...
    def get_gradients(self, data):
        '''get the full gradients using a mini-batch of data. batch size here is set to be 50.
        ...
        if num_samples < 50:
            ...
            grads = process_sparse_grad2(model_grads)
        else:
            input_data = process_x(data['x'][0:50])
            target_data = process_y(data['y'][0:50])
            with self.graph.as_default():
                model_grads = self.sess.run(self.grads,
                feed_dict={self.features: input_data, self.labels: target_data})
            grads = process_sparse_grad2(model_grads)
        return min(num_samples, 50), grads
