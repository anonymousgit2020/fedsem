...
class Model(ABC):
    ...
    def __init__(self, seed, lr, optimizer=None):
        ...
        with self.graph.as_default():
            ...
            self.features, self.labels, self.train_op, self.eval_metric_ops, self.loss, self.pred_ops, self.c_f1 = self.create_model()
    ...
    def test(self, data):
        ...
        with self.graph.as_default():
            tot_acc, loss, pred_ops, f1 = self.sess.run(
                [self.eval_metric_ops, self.loss, self.pred_ops, self.c_f1],
                feed_dict={self.features: x_vecs, self.labels: labels}
            )
        acc = float(tot_acc) / x_vecs.shape[0]             
        f1 = np.nan_to_num( f1 )
        return {ACCURACY_KEY: acc, 'loss': loss, "f1":f1}
