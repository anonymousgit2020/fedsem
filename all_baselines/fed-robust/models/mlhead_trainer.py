...
def mlhead_print_totloss(k, eval_every, rounds, prefix, loss, cluster, stack_list):
    w_s = [c[0] for c in cluster if c[0] > 1]
    tot_ws = np.sum(w_s)
    w_s = [x / tot_ws for x in w_s]
    if loss.ndim == 0:
        micro_loss = np.average(loss)
        macro_loss = np.average(loss)
    else:
        micro_loss = np.average(a = loss, weights=w_s)
        macro_loss = np.average(a = loss, weights=None)
        
    print('micro test_%s: %g' % (prefix, micro_loss) )
    print('macro test_%s: %g' % (prefix, macro_loss) )
    save_metric_csv(k+1, micro_loss, stack_list, cluster)
...
class MlheadTrainer():
    ...
    def __init__(self, args, users, groups, train_data, test_data): 
        ...
        tf.set_random_seed(5555 + args.seed)
        ...
        client_ids, client_groups, client_num_samples = TrimServer(client_model).get_clients_info(self.clients)
        ...
        self.head_server_stack = [TrimServer(client_model) for _ in range(max(args.num_clusters, 1))]
        print("--- Do training and initilized clusting server---")
        self.mlhead_cluster = Mlhead_Clus_Server(client_model, args.dataset, args.model, \
                                                 args.num_clusters, len(self.clients), args.mean_est)
        ...
        for k in range(self.num_rounds):
            ...
            if prev_score is None: # This is the first iteration
                ...
                # cluster is in this form: a list of (num_clients, clients), client is a array of client model.             
                    ...
                    prev_score = len(learned_cluster)
            ...
            for c_idx, group in enumerate(learned_cluster):
                ...
                if args.weight_mode == "no_size":
                    server.update_trimmean(active_clients)
            ...
            mlhead_print_totloss(
                k, self.eval_every, self.num_rounds, "loss",  best_kept, learned_cluster, stack_list)
            ...
            if not args.num_clusters == -1 and not k == (self.num_rounds -1) and (k + 1) % args.update_head_every == 0:
                ...
                _, learned_cluster = self.mlhead_cluster.run_clustering(prev_score, c_wts)# cwts is N (clients) x x_d
