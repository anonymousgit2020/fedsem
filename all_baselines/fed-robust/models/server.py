...
class TrimServer:
    ...
    def download_all(self, clients):
        for c in clients:
            c.model.set_params(self.model)
        
    def get_var_vector(self, client, variable):
        model = client.model.get_params()
        vec = model[variable].astype(np.float64)
        return vec.flatten()
    
    def update_trimmean(self, clients):
        # Totally,3 levels of values to construct an big array
        # client level, model level, layer weights level
        #1. a bottom method to loop through model level to 
        # flatten all the layer to a vector
        #2. a method get the model params and feed into above method
        #3. a method to get the trimmed mean
        #4. a method to reconstruct the solution from the above result
#         x_model_list = [c.model.get_params() for c in clients]
        num_variables = len(self.model)
        shape = [v.shape for v in self.model]
        base = [0.] * num_variables
        for i in range(num_variables):
            func = lambda client: self.get_var_vector(client, i)
            X = np.array([func(client) for client in clients])           
            base[i] = np.array( stats.trim_mean(X, 0.2, axis=0) ).reshape(shape[i])
